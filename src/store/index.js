...
export default new Vuex.Store({
    ...
    REMOVE_LOCATION: (state, index) => {
      if (window.confirm("Are you sure you want to delete this item?")) {
        for (let i = 0; i < state.locations.length; i++) {
          if (state.locations[i].id == index) {
            state.locations.splice(i, 1)
          }
        }
      }
    },
    REMOVE_REVIEW: (state, index) => {
      if (window.confirm("Are you sure you want to delete this item?")) {
        for (let i = 0; i < state.reviews.length; i++) {
          if (state.reviews[i].id == index) {
  ...
  actions: {
    ...
    removeLocation: (context, index) => {
      context.commit('REMOVE_LOCATION', index);
    },
    removeReview: (context, index) => {
      context.commit('REMOVE_REVIEW', index);
    }
  },
  modules: {}
});